  ...
  render() {
    return (
      <div>
        ...
        >     
          <CartesianGrid />
          <XAxis type="category" dataKey="commonName" name="Countries"/>
          <YAxis type="number" dataKey="population" name="Population" />
          <Tooltip cursor={{ strokeDasharray: '3 3' }} />
          <Scatter name="Population By Countries" data={this.state.thirdData} fill="#8884d8">
          </Scatter>
              </ScatterChart>
        </div>
        </ResponsiveContainer>
          ...
          <PieChart width={400} height={500} >
            ...
            >
              {this.state.covData.map((val, index) => (
                <Cell key={`cell-${val.cases}`} fill={this.COLORS[index % this.COLORS.length]}/>
              ))}
            </Pie>
          </PieChart>
        </ResponsiveContainer>
