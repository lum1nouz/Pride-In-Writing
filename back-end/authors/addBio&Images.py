...
for i in authFinalData.index:
    ...
    for j in imageData.index:
        if str(imageData["name"][j]).find(author_name) != -1:
            author_list.append(
                {
                    "Name": authFinalData["Name"][i],
                    "Year Born": authFinalData["Year Born"][i],
                    "Nationality": authFinalData["Nationality"][i],
                    "Genre": authFinalData["Genre"][i],
                    "Notable Works": authFinalData["Notable Works"][i],
                    "OnTour": authFinalData["OnTour"][i],
                    "Link": imageData["image"][j],
                    "Summary": authFinalData["Summary"][i],
                    "id": authFinalData["id"][i],
                    "BookConnections": authFinalData["BookConnections"][i],
                }
            )
            found = True
    if not found:
