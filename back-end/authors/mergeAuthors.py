...
lgbtData = pd.read_csv(r'./filtered_authors.csv') #Name,Lifetime,Nationality,Genre,Notable Works
counter = 0
author_list = []
for i in lgbtData.index:
    ...
    for j in prhData.index:
        if(str(prhData["Name"][j]).find(author_name) != -1):
            author_list.append({'Name' : prhData["Name"][j], 'Year Born' :lgbtData["Lifetime"][i], 'Nationality' : lgbtData["Nationality"][i],  'Genre' : lgbtData["Genre"][i], 'Notable Works' : lgbtData["Notable Works"][i], 'OnTour' : prhData["OnTour"][j],'Link': prhData["Link"][j], 'Summary': prhData["Summary"][j]})
